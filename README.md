...
| `StaticStr.hpp` | `avp::StaticStr` — what the **C++** `sprintf_static()` returns: a move-only claim on the shared buffer for the result’s lifetime, so nesting/reusing it asserts instead of silently corrupting. Converts implicitly to `const char *` (a `String`/class parameter needs explicit `.c_str()`). C keeps the plain `const char *` prototype. Detection is `NDEBUG`-only, zero cost in release. |
