  ...
  class Array {
   ...
   public:
    ...
    Array(const Array<T1, Length> &rhs) {
      for(size_t i; i < Length; ++i) Data[i] = rhs[i];
    }
...
#define SELF_OP_T(Class, ...)                                                           \
  ...
  const Class &operator __VA_ARGS__##=(const T1 &rhs) {                                 \
    ...
    // const Array &operator=(const T *p) { for(size_t i=0; i < Length; i++) Data[i] = p[i];  return *this;}
  ...
  template <typename T, size_t Length>
  class PlacedArray : public Array<T, Length> {
   protected:
    size_t Nfilled;
